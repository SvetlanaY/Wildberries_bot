  ...
  #  url = 'http://api:8000/api/photo'
    ...
    if not os.path.isfile(f'./df/{user_id}.csv') or res_id != 0:              
                  ...
                  #  preprocessed_comments = word_cloud_output(file_name,output_name,user_id)           
                   ...
                   # button_3 = KeyboardButton(text[2])
                    ...
                    #button_5 = KeyboardButton(text[4])
                        ...
                        await message.answer('Нажми на слово или напиши что-то своё',reply_markup=markup)
        ...
        if not os.path.isfile(f'./similarity/{user_id}_{word}.csv'):
            await message.reply("К сожалению, мы ничего не нашли по твоему запросу, попробуй изменить тему или тональность отзывов")
        ...
        #  кнопки sentiments
        button_sent_p = InlineKeyboardButton('Положительные отзывы', callback_data=f'sent_{word_pos}')
        button_sent_n = InlineKeyboardButton('Отрицательные отзывы', callback_data=f'sent_{word_neg}')
        button_sent_all = InlineKeyboardButton('Все отзывы', callback_data=f'sent_{word_neu}')
...
async def callback_top(call:types.CallbackQuery):
    ...
    text = similar_comments(action,nlp,user_id)
    #if not text:
   #     await call.message.answer('К сожалению, мы ничего не нашли по твоему запросу, попробуй изменить тему или тональность отзывов')
