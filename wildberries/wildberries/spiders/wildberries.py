...
class WildberriesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        data = {
            ...
            "order": "dateDesc"}
        yield JsonRequest(url='https://public-feedbacks.wildberries.ru/api/v1/feedbacks/site',
                          data=data, callback=self.parse_my_url)
    ...
    def parse_my_url(self, response):
        ...
        for i in range(self.count_comment):
            try:
                ...
                res.append(data_from_json['feedbacks']
                           [i].get('votes').get('pluses'))
                res.append(data_from_json['feedbacks']
                           [i].get('votes').get('minuses'))
                res.append(data_from_json['feedbacks']
                           [i].get('productValuation'))
                res.append(data_from_json['feedbacks'][i].get(
                    'productDetails').get('productName'))
                res.append(data_from_json['feedbacks'][i].get(
                    'productDetails').get('brandName'))
                result[i] = res
